...
const BlogCard = ({
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: blog.title,
        text: blog.content.substring(0, 100) + '...', // Short preview of the content
        url: window.location.href, // Assuming the current page URL is the blog link
  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex-1 flex flex-col">
            ...
            >
              <i
                className={`fas fa-bookmark ${
                  blog.isBookmarked ? "text-blue-500 animate-scale-bounce" : ""
                }`}
              ></i>
            </button>
            <span>
              <i className="fas fa-heart text-red-500"></i> {blog.likes}
            </span>
        ...
        <p className="text-gray-600 mb-4 text-justify whitespace-pre-wrap">
          {blog.content}
        </p>
        ...
        <div className="mt-auto">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4 text-gray-500">
                ...
                <button
                className="text-gray-600 hover:text-red-500 hover:scale-150  transition-transform duration-300 flex items-center space-x-2 "
                onClick={user ? () => handleLike(blog.id) :()=> toast.error("Please login to like this blog")}
              >
                <i
                  className={`fas fa-heart 
                                ${
                                  blog.isLiked
                                    ? "text-red-500 animate-scale-bounce"
                                    : ""
                                } 
              ...
              <button
              onClick={handleShare}
              className="hover:text-blue-500  transition-all duration-300 hover:scale-150  "
            >
